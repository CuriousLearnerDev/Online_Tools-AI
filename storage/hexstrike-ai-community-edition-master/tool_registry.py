...
TOOLS: Dict[str, dict] = {
    ...
    "naabu": {
        "desc": "ProjectDiscovery Naabu — high-speed TCP port enumeration (CONNECT/SYN)",
        "endpoint": "/api/tools/naabu",
        "method": "POST",
        "category": "network_recon",
        "params": {},
        "optional": {
    ...
    "fscan": {
        "desc": "Fscan 内网综合扫描（存活、端口、服务/指纹、POC；需授权；target=-h）",
        "endpoint": "/api/tools/fscan",
        "method": "POST",
        "category": "network_recon",
        "params": {},
        "optional": {
    ...
    "kscan": {
        "desc": "Kscan — 端口/协议指纹/FOFA/spy；可选 hydra（需授权）",
        "endpoint": "/api/tools/kscan",
        "method": "POST",
        "category": "network_recon",
        "params": {},
        "optional": {
    ...
    "gobuster": {
        ...
        "optional": {"mode": "dir", "wordlist": "..\common.txt", "additional_args": ""},
        "effectiveness": 0.90,
    },
    "ffuf": {
        ...
        "category": "web_recon",
        "params": {"url": {"required": True}},
        "optional": {"wordlist": "..\common.txt", "mode": "directory", "match_codes": "200,204,301,302,307,401,403", "additional_args": ""},
        "effectiveness": 0.90,
    },
    "feroxbuster": {
        ...
        "category": "web_recon",
        "params": {"url": {"required": True}},
        "optional": {"wordlist": "..\common.txt", "threads": 10, "additional_args": ""},
        "effectiveness": 0.85,
    },
    "katana": {
    ...
    # ---- Web Vuln ----
    "nuclei": {
        "desc": "Template-based vulnerability scanner（template 留空时服务端自动使用 NUCLEI_TEMPLATES_DIR 或 ../nuclei/nuclei-templates）",
    ...
    "pocbomber": {
        "desc": "PocBomber PoC/exp batch scanner（必填 target=单 URL 即 -u；批量用 file；show=true 时 target 可填任意占位；需本地脚本/POCBOMBER_SCRIPT）",
        "endpoint": "/api/tools/pocbomber",
        "method": "POST",
        "category": "web_vuln",
        "params": {"target": {"required": True}},
        "optional": {
    ...
    "springboot-scan": {
        "label": "SpringBoot-Scan",
        "desc": "SpringBoot-Scan：Spring 信息泄露/漏洞/敏感文件；单 URL 用 url 或 target(映射-u)；help=true 可看参数说明；工具名 springboot-scan",
        "endpoint": "/api/tools/springboot-scan",
        "method": "POST",
        "category": "web_vuln",
        "params": {},
        "optional": {
    ...
    "urlfinder": {
        "desc": "URLFinder — extract JS and URLs from pages (sensitive/hidden API hunting)",
        "endpoint": "/api/tools/urlfinder",
        "method": "POST",
        "category": "web_recon",
        "params": {},
        "optional": {
    ...
    "dirb": {
        ...
        "category": "essential",
        "params": {"url": {"required": True}},
        "optional": {"wordlist": "..\common.txt", "additional_args": ""},
        "effectiveness": 0.78,
    },
    "hakrawler": {
    ...
    # tool_registry.py 里 TOOLS 字典的一部分
    "ehole": {
        "desc": "EHole 指纹识别工具：快速从大量资产中定位重点系统 (OA/VPN/WebLogic 等)",
        "endpoint": "/api/tools/ehole",
        "method": "POST",
        "category": "osint",
        "params": {
            # 单域名扫描：固定走 finger -u
        ...
        "optional": {
            "config": "",              # --config 路径（可选）
            "thread": 100,             # finger -t/--thread（可选）
            "proxy": "",               # finger -p/--proxy（可选）
            "output": "",              # finger -o/--output（可选，json/xlsx）
            "additional_args": "",     # 其它 CLI 参数（可选）
        },
        "effectiveness": 0.80,
    },
    "burpsuite": {
    ...
    "api_fuzzer": {
        ...
        "optional": {"method": "GET", "wordlist": "..\common.txt", "endpoints": []},
        "effectiveness": 0.80,
        "parent_tool": "ffuf",
    },
...
# Merge Tongling tools_config + toollist CLI entries (统领 storage)
try:
    from server_core.tongling_tool_catalog import get_tongling_registry_entries

    TOOLS.update(get_tongling_registry_entries(skip_aliases=TOOLS.keys()))
except Exception as _tongling_reg_err:
    logger.warning("Tongling tool registry merge skipped: %s", _tongling_reg_err)
...
CATEGORIES = {
    ...
    "tongling": "Tongling storage tools (tools_config + toollist CLI)",
}
...
_INTENT_KEYWORDS: Dict[str, List[str]] = {
    ...
    "web_vuln": [
        "vuln", "vulnerability", "nuclei", "afrog", "pocbomber", "poc", "cnvd",
        "sql", "sqli", "xss",
        "injection", "exploit", "dalfox", "traversal",
        "springboot", "spring boot", "actuator", "springboot-scan",
    ],
    "exploitation": [
        "exploit", "c", "msf", "payload", "msfvenom", "exploit-db",
    ...
    "osint": [
        ...
        "gau", "waybackurls", "urlfinder", "js", "接口",
        "ehole", "fingerprint", "fofa", "fofaext", "指纹"
    ],
    "binary": [
...
_TOOL_NAME_ALIASES: Dict[str, str] = {
    "springboot_scan": "springboot-scan",
    "SpringBoot-Scan": "springboot-scan",
    "SpringBoot_Scan": "springboot-scan",
    "jwt-tool": "jwt_tool",
    "hash_identifier": "hash-identifier",
    "tongling_run": "tongling-run",
    "tongling_catalog": "tongling-catalog",
}
...
def get_tool(name: str) -> Optional[dict]:
    ...
    if not name:
        return None
    n = name.strip()
    canon = _TOOL_NAME_ALIASES.get(n) or _TOOL_NAME_ALIASES.get(n.lower())
    if canon:
        return TOOLS.get(canon)
    return TOOLS.get(n)
